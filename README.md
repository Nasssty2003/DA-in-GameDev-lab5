...
Изучая график по первоначальным данным, можно увидеть, что при обучении Episode Length должен возрастать, Cumulative Reward также увеличивается, но не должен превышать 1 по вертикальной оси. Возрастает и в свою очередь Policy Loss. А вот Value Loss наоборот убывает.
Изменении заданных параметров (batch_size, buffer_size, num_players, epsilon, lambd), как оказалось, не сильно влияют на графики. Из 4 изменения пришлись только на параметр num_players. После экспериментов именно с этим параметром скорость обучения замедлилась, следовательно обучение стало занимать больше времени, чем по другим параметрам. 
